...
def predict(config, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            Attention4, out = gen(x)

            #threshold = 0.55
            #att = torch.where(att >= threshold, torch.tensor(1.0, device=att.device),
                               #torch.tensor(0.0, device=att.device))
            ...
            att_ = Attention4.cpu().numpy()[0]* 255
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained', type=str, required=True,
                        default='./results/000136/models/gen_model_epoch_161.pth')
